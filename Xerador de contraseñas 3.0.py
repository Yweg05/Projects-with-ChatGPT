...
caracteres = ['abcdefghilmnñopqrstuvwxz',
...
class Contrasinal:
    ...
    def add1():
        ...
        l_name = tk.Label(ventana, text="Lonxitude: ")
        l_name.grid(row=2, column=1)
        Contrasinal.e_length = tk.Entry(ventana)
        Contrasinal.e_length.grid(row=2, column=2, columnspan=2, pady=5)
        
        l_name = tk.Label(ventana, text="Nivel de seguridade: ")
        l_name.grid(row=3, column=1)
        Contrasinal.e_security = ttk.Combobox(ventana, values=['1','2','3','4'], state='readonly')
    ...
    def add2():
        ...
        for x in Contrasinal.all:
            if name == x.name:
                ...
                l_e2 = tk.Label(ventana, text="Xa existe un contrasinal con ese nome").pack()
                b_menu.pack()
                break
        else:
            try:
                ...
                if 0 < length < 26:
                        ...
                        Contrasinal.generated_password = Contrasinal.create_password(material, length)
                        ...
                        l_add1 = tk.Label(ventana, text=f"O contrasinal xerado é {Contrasinal.generated_password}")
                        l_add1.grid(row=2, column=1, columnspan=2, pady=5, padx=10)
                            
                        b_add1 = tk.Button(ventana, text="Confirmar contrasinal", command=Contrasinal.confirm)
                        b_add1.grid(row=3, column=1, pady=3, padx=1)
                        b_add2 = tk.Button(ventana, text="Xerar unha nova", command=Contrasinal.create_loop)
                        b_add2.grid(row=3, column=2, pady=3, padx=1)       
                           
                else:
                    clean()
                    l_e1 = tk.Label(ventana, text="").pack()
                    l_e2 = tk.Label(ventana, text="A lonxitude debe ser maior ca 0 e máximo 25").pack()
                    b_menu.pack()
            
            except ValueError:
                clean()
                l_e1 = tk.Label(ventana, text="").pack()
                l_e2 = tk.Label(ventana, text="Escriba un número enteiro na lonxitude").pack()
                b_menu.pack()
    
    # Parte 3: Método por si o usuario quere cambiar o contrasinal coas mesmas especificacións
    @staticmethod
    def create_loop():
        Contrasinal.generated_password = Contrasinal.create_password(Contrasinal.material, Contrasinal.length)
    ...
    def delete2():
        ...
        for index, x in enumerate(Contrasinal.all):
            if name == x.name:
                ...
                clean()        
                l_e1 = tk.Label(ventana, text="").pack()
                l_e2 = tk.Label(ventana, text="Contrasinal eliminado exitosamente").pack()
                b_menu.pack()
                break
        else:
            clean()        
            l_e1 = tk.Label(ventana, text="").pack()
            l_e2 = tk.Label(ventana, text="Non hai ningún contrasinal con ese nome").pack()
            b_menu.pack()
    ...
    def show():
        ...
        if not Contrasinal.all:
            ...
            for x, instancia in enumerate(Contrasinal.all):
                l_show2 = tk.Label(ventana, text=f"> {instancia.name}: {instancia.contrasinal}")
...
def menu():
    ...
    l_menu = tk.Label(ventana, text="Qué quere facer?").grid(row=0, column=1, padx=25, pady=25)

    b_add = tk.Button(ventana, text="Engadir un novo contrasinal", command=Contrasinal.add1)
    b_add.grid(row=1, column=1, padx=25, pady=5)

    b_del = tk.Button(ventana, text="Eliminar un contrasinal xa existente", command=Contrasinal.delete1)
    b_del.grid(row=2, column=1, padx=25, pady=5)

    b_show = tk.Button(ventana, text="Amosar todos os contrasinais", command=Contrasinal.show)
